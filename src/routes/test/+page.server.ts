...
function validateShareTarget(
	shareTarget: unknown,
	manifestUrl: string
): asserts shareTarget is ShareTarget {
	if (shareTarget == undefined) {
		throw error(
			400,
			`Web App Manifest located at '${manifestUrl}' does not have 'share_target' member`
		);
	} else if (typeof shareTarget !== 'object') {
		throw error(
			400,
			`Web App Manifest located at '${manifestUrl}' does not have valid 'share_target'. Found ${shareTarget} value.`
		);
	}
}
...
export const load: import('./$types').PageServerLoad = async ({ url, fetch }) => {
	...
	try {
		...
		if (err instanceof SyntaxError) {
			throw error(
				400,
				`'manifest' URL '${manifestUrl}' points to invalid JSON. Content type was ${
					response.headers.get('Content-Type') ?? 'not sent'
				}`
			);
		}
		throw err;
	...
	const action = new URL(shareTarget.action, manifestUrl).toString();
